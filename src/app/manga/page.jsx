...
export default function MangaPage() {
  ...
  const getMangaList = useCallback(async () => {
    setMangaList({ error: false, loading: true, data: [] });
    setVisibleCount(PAGE_SIZE); // reset on new filter
    ...
    try {
      ...
      const result = await response.json();
      setMangaList({ error: false, loading: false, data: result.mangaList });
    } catch (error) {
      console.error(error.message);
      setMangaList({ error: true, loading: false, data: [] });
    }
  ...
  useEffect(() => {
    if (isStateLoaded && userId !== undefined) {
      saveStateToStorage(userId, { searchTerm, selectedGenre, selectedStatus });
    }
  }, [searchTerm, selectedGenre, selectedStatus, isStateLoaded, userId]);

  useEffect(() => {
    if (isStateLoaded) getMangaList();
  ...
  const handleObserver = useCallback(
    (entries) => {
      ...
      if (target.isIntersecting && !mangaList.loading) {
        setVisibleCount((prev) => {
          if (prev >= mangaList.data.length) return prev;
  ...
  const handleReset = () => {
    ...
    setSelectedGenre("");
    setSelectedStatus("");
    if (userId) clearStateFromStorage(userId);
  };

  const visibleManga = mangaList.data.slice(0, visibleCount);
  ...
  return (
    <div className="w-full max-w-5xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
      ...
      {mangaList.error ? (
        ...
        <div className="w-full mt-10 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 sm:gap-4 md:gap-5">
    ...
    >
      <div className="flex flex-col gap-1.5 sm:gap-2 w-full">
        <div className="overflow-hidden rounded-lg border border-gray-700 w-full aspect-[9/13] relative bg-gray-900">
          {!imageLoaded && (
            <Skeleton className="absolute inset-0 bg-gray-300/30 w-full h-full rounded-lg" />
          )}
          <Image
